...
export class CheckoutUserFormComponent {
  ...
  constructor(
    ...
    this.dynamicFormConfig$ = store.pipe(
      ...
      take(1), // only do this on initial load, and not on changes to the User data, otherwise we'll be effectively trying to rebuild the form on each change. Currently, the form component doesn't properly watch the config input for changes to then update the form controller's group's control's values or recreate the whole form all together... a lot to be added to DynamicFormModule, come time
      map(checkoutUser => ({
        inputs: [
          {
            type: 'text',
